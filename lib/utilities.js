...
import {MAX_DECIMALS} from '../constants/game'
...
const FLOAT_FIX_MULTIPLIER = Math.pow(10, MAX_DECIMALS)
/**
 * @param {number} num
 * @returns {number}
 */
export function floatFix(num) {
  return Math.round(num * FLOAT_FIX_MULTIPLIER) / FLOAT_FIX_MULTIPLIER
}
...
const calculateFadedColor = (color, opacity) => {
  if (color[0] === '#') {
    if (color.length < 6) {
      const frag = color.substr(1)
      color = '#' + (frag + frag + frag).substr(0, 6)
    }
...
/**
 * This wil return the center of the element on the page, not the origin
 * @param {*} e
 ...
 * @returns {number}
 */
export function applyTimeBoostToScore(score, timeToAnswerMS) {
  return Math.max(0, Math.floor(score + score / (Math.max(1, timeToAnswerMS) / 1000))) // boosted by the inverse number of seconds
