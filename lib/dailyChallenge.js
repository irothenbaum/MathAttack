...
export function scheduleReminders(gameSettings) {
  ...
  // if the daily challenge is disabled, the time will be undefined
  if (typeof gameSettings.dailyChallengeTime !== 'number') {
    return
  }
...
function scheduleSingleReminder(gameSettings, id) {
  ...
  NotificationHelper.Instance().scheduleLocalNotification(
    ...
    moment().add(REMINDER_DAYS_MAP[id], 'day').startOf('days').add(gameSettings.dailyChallengeTime, 'minutes').toDate(),
